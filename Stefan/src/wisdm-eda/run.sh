...
if [ -z "$1" ]; then
  ...
  echo "Usage: ./run.sh path/to/your_script.py"
  exit 1
fi

# 1. Setup Paths
PYTHON_SCRIPT_PATH="$1"
SCRIPT_DIR=$(dirname "$PYTHON_SCRIPT_PATH")   # The folder containing the python script
SCRIPT_NAME=$(basename "$PYTHON_SCRIPT_PATH") # The actual filename (e.g., eda.py)
